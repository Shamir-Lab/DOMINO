...
def pf_filter(params):
    G_modularity, G_optimized, G_original, cur_cc, i_cur_cc, pertubed_nodes, perturbation_factor = params
    ...
    if len(cur_cc) < 4 or not (len(pertubed_nodes_in_cc) > perturbation_factor or len(pertubed_nodes_in_cc)/float(len(cur_cc))>=0.2):
        ...
        score=hypergeom.sf(len(pertubed_nodes_in_cc), len(G_original.nodes), len(pertubed_nodes),
                                       len(cur_cc)) \
